...
import { CyberSimulatorPage } from './components/CyberSimulatorPage';
import { AdvancedHackerTyperPage } from './components/AdvancedHackerTyperPage';
import { KernelIdPage } from './components/KernelIdPage';
...
import { Shield, Home, BookOpen, Globe, Award, Mail, FileText, Menu, X, ShieldCheck, HelpCircle, Terminal, Code2, Fingerprint } from 'lucide-react';
import { KernelAxisLogo } from './components/KernelAxisLogo';
import { playSynthBeep } from './lib/audio';
import { Analytics } from '@vercel/analytics/react';
import { SpeedInsights } from '@vercel/speed-insights/react';

type ViewState = 'home' | 'learn' | 'tools' | 'map' | 'about' | 'contact' | 'privacy' | 'terms' | 'questions' | 'simulator' | 'hacker-typer' | 'kernel-id';
...
export default function App() {
  ...
  const getActiveView = (path: string): ViewState => {
    ...
    if (path.startsWith('/kernel-id') || path.startsWith('/id-card') || path.startsWith('/operator-id')) return 'kernel-id';
    if (path.startsWith('/cyber-simulator') || path.startsWith('/cyber-security')) return 'simulator';
    if (path.startsWith('/kernel-hacker-typer') || path.startsWith('/advanced-hacker-typer') || path.startsWith('/hacker-typer')) return 'hacker-typer';
  ...
  useEffect(() => {
    ...
    if (path === '/map') {
      ...
      title = 'Cybersecurity Questions | Kernel Axis';
      description = 'Test your cybersecurity knowledge with interactive practice questions and explanations.';
    } else if (path.startsWith('/kernel-id') || path.startsWith('/id-card') || path.startsWith('/operator-id')) {
      title = 'Kernel ID — Cyber Security Virtual ID Badge | Kernel Axis';
      description = 'Create and download your customized Kernel Axis Cyber Security Operator ID card. Choose your clearance level, specialization, custom photo, and save a high-res virtual badge.';
    } else if (path.startsWith('/kernel-hacker-typer') || path.startsWith('/advanced-hacker-typer') || path.startsWith('/hacker-typer')) {
      title = 'Kernel Hacker Typer | Kernel Axis';
      description = 'Interactive hacker typing terminal simulator by Kernel Axis. Stream authentic kernel code in real-time, customizable retro themes, sound effects, and hacker controls.';
    }
  ...
  const handleNavigate = (view: ViewState) => {
    ...
    else if (view === 'simulator') navigate('/cyber-simulator');
    else if (view === 'hacker-typer') navigate('/kernel-hacker-typer');
    else if (view === 'kernel-id') navigate('/kernel-id');
  };
  ...
  const navLinks: { id: ViewState; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'kernel-id', label: 'Kernel ID', icon: <Fingerprint className="w-3.5 h-3.5" /> },
    { id: 'map', label: 'Threat Map', icon: <Globe className="w-3.5 h-3.5" /> },
    { id: 'simulator', label: 'Simulator', icon: <Terminal className="w-3.5 h-3.5" /> },
    { id: 'hacker-typer', label: 'Kernel Hacker Typer', icon: <Code2 className="w-3.5 h-3.5" /> },
  ];

  const mobileNavLinks: { id: ViewState; label: string; icon: React.ReactNode }[] = [
    ...navLinks,
    { id: 'questions', label: 'TESTS', icon: <HelpCircle className="w-3.5 h-3.5" /> },
  ];
      ...
      <main className={`w-full flex-grow relative ${activeView === 'map' ? 'h-full overflow-hidden' : ''}`}>
        <AnimatePresence mode="wait">
          <Routes location={location}>
            ...
            <Route path="/id-card" element={<Navigate to="/kernel-id" replace />} />
            <Route path="/operator-id" element={<Navigate to="/kernel-id" replace />} />

            <Route path="/cyber-simulator" element={<CyberSimulatorPage />} />
            <Route path="/cyber-security" element={<CyberSimulatorPage />} />
            <Route path="/kernel-hacker-typer" element={<AdvancedHackerTyperPage />} />
            <Route path="/advanced-hacker-typer" element={<AdvancedHackerTyperPage />} />
            <Route path="/hacker-typer" element={<AdvancedHackerTyperPage />} />
          ...
          <div className="max-w-[1536px] mx-auto grid grid-cols-2 md:grid-cols-12 gap-x-6 gap-y-6 md:gap-8">
            ...
            <div className="col-span-1 md:col-span-3 space-y-2 md:space-y-3">
              ...
              <ul className="space-y-1 md:space-y-2 font-sans text-xs">
                ...
                <li>
                  <button onClick={() => handleNavigate('kernel-id')} className="text-zinc-400 hover:text-[#00ff88] transition-colors cursor-pointer text-left w-full block py-2 md:py-0.5">Kernel ID Badge</button>
                </li>
                <li>
                ...
                <li>
                  <button onClick={() => handleNavigate('simulator')} className="text-zinc-400 hover:text-[#00ff88] transition-colors cursor-pointer text-left w-full block py-2 md:py-0.5">Simulator</button>
                </li>
                <li>
                  <button onClick={() => handleNavigate('hacker-typer')} className="text-zinc-400 hover:text-[#00ff88] transition-colors cursor-pointer text-left w-full block py-2 md:py-0.5">Kernel Hacker Typer</button>
                </li>
              </ul>
            </div>
